(function (factory) {
        ...
        for (var i = 0; i < 9; i++) { // this loop is for is weeks (rows)
            for (var j = 0; j < _.$label.days.length; j++) { // this loop is for weekdays (cells)
                if (day <= _.monthLength && (i > 0 || j >= _.startingDay)) {
                    ...
                    markup += '<div class="day" role="button"  data-date-val="' + thisDay + '"><div class="w-100 cs-the-day">' + day + '</div></div>';
                    day++;
                } else {
                    markup += '<td>';
                }
    ...
    EvoCalendar.prototype.addEventIndicator = function (active_date, type,name, price, discount) { 

        var _ = this,
            htmlToAppend, thisDate = _.$elements.innerEl.find('[data-date-val="' + active_date + '"]');
        if (thisDate.find('span.cs-event-indicator').length === 0) {

            if (discount === '') { 
                thisDate.addClass('cs-calender-active cs-event-' + type).attr({
            ...
            var getCustomAppend = '';
            getCustomAppend += '<span class="cs-event-indicator" data-status="' + type + '"></span>';
            if ( discount === '') {
                getCustomAppend += '<small class="cs-calender-price">' + price + '</small>';
            } else { 
                getCustomAppend += '<small class="cs-calender-price"><del>' + price + '</del>' + discount + '</small>';
            }
            getCustomAppend += '<small class="cs_date-type">'+name+'</small>';

            thisDate.append(getCustomAppend);
        }

        if (thisDate.find('span.cs-event-indicator > .type-bullet > .type-' + type).length === 0) {
            htmlToAppend = '<div class="type-bullet"><div class="type-' + type+'"></div></div>';
            thisDate.find('.cs-event-indicator').append(htmlToAppend);
        }
    }
    ...
    EvoCalendar.prototype.removeEventIndicator = function (active_date, type, price) {
        ...
        if (_.$elements.innerEl.find('[data-date-val="' + active_date + '"] span.cs-event-indicator').length === 0) {
            return;
        }
        ...
        if (eventLength === 0) {
            _.$elements.innerEl.find('[data-date-val="' + active_date + '"] span.cs-event-indicator > .type-bullet > .type-' + type).parent().remove();
        }
    }
    ...
    EvoCalendar.prototype.buildEventIndicator = function () {
        ...
        // prevent duplication
        _.$elements.innerEl.find('.calendar-day > day > .cs-event-indicator').empty();
        ...
        for (var i = 0; i < _.options.calendarEvents.length; i++) {
            for (var x = 0; x < _.monthLength; x++) {
                ...
                if (active_date == _.options.calendarEvents[i].date) {
                    _.addEventIndicator(active_date, _.options.calendarEvents[i].type, _.options.calendarEvents[i].name, _.options.calendarEvents[i].price, _.options.calendarEvents[i].discount);
                } else if (_.options.calendarEvents[i].everyYear) {
